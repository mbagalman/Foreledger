...
> **Amended 2026-06-12 — `as_of` terminology.** The shipped `as_of(origin)` surface is an *origin-scoped* filter over the archive's current state (FR-4.1); "bitemporal" is reserved for the deferred transaction-time/actual-vintage query surface, whose axes (`ingested_at`, `actual_recorded_at`) the data model retains. Wording updated here and in the plan/PRD accordingly.
