...
The product is a lightweight forecast archive that sits between training-time evaluation tools (which generate this structure but discard it) and ML monitoring platforms (which store predictions but key them on the wrong axis). It ingests each run's forecasts, stores them efficiently, and exposes horizon-keyed evaluation and origin-scoped as-of slicing as first-class operations. The contribution is integration and ergonomics, not new math.
